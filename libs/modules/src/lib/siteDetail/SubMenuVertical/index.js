...
const MailSidebar = () => {
  ...
  const dataList = useMemo(() => {
    return Object.values(SITE_DETAIL_MENU_CONFIG[mainMenu].subMenu).map(
      (item) => {
        return {
          id: item.key,
          name: item.text,
          alias: item.key,
          icon: item.icon || "file-archive",
        };
      }
    );
  }, []);
  ...
  return (
    <>
      {SITE_DETAIL_MENU_CONFIG[mainMenu]?.subMenu && (
        <SubMenuVerticalWrapper bordered={false}>
          <StyledMailSidebarScrollbar>
            <StyledMailSidebarContent>
              <StyledMailSidebarList
                component="nav"
