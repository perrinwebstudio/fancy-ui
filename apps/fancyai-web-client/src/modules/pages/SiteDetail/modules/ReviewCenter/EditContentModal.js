...
import { StyledCardCalendarWrapper, StyledCardRow, StyledModalInfoWrapper, StyledModalOrignalContentWrapper, StyledOriginalContent, StyledTagWrapper } from './index.styled';
...
const EditContentModal = ({ open, onClose, item }) => {
  ...
  return <>
    <Modal footer={<></>} open={open} closeIcon={null} width={1475} closable={false}>
      ...
      <StyledModalInfoWrapper>
        ...
        <Typography.Title level={5} style={{ marginBottom: 0, fontWeight: 500 }}>Review And Approval Required For Content Update</Typography.Title>
      </StyledModalInfoWrapper>

      <StyledModalOrignalContentWrapper>
        <Typography style={{ color: '#626D80', fontSize: '18px' }}>Original Content</Typography>
        <StyledOriginalContent>{itemData?.data?.data}</StyledOriginalContent>
      </StyledModalOrignalContentWrapper>

      {!isLoading && (<StyledModalOrignalContentWrapper>
        <Typography style={{ fontSize: '18px' }}>Updated Content</Typography>
